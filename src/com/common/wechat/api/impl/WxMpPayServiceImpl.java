...
import com.common.wechat.common.util.crypto.WxCryptUtil;
import com.common.wechat.common.util.http.Utf8ResponseHandler;
import com.common.wechat.common.util.xml.XStreamInitializer;
import com.common.wechat.api.WxMpPayService;
...
public class WxMpPayServiceImpl implements WxMpPayService {
    ...
    private static final Logger log = Logger.getLogger(WxMpPayServiceImpl.class);
    ...
    public WxMpPrepayIdResult getPrepayId(final Map<String, String> parameters) {
        ...
        log.info("微信预支付订单request"+packageParams.toString());
        ...
        for (Map.Entry<String, String> para : packageParams.entrySet()) {
            ...
            if ("detail".equals(para.getKey())) {
                value = String.format("<![CDATA[%s]]>", value);
            }
            request.append(String.format("<%s>%s</%s>", para.getKey(), value, para.getKey()));
        }
    ...
    @Override
    public Map<String, String> getJsapiPayInfo(String openId, String outTradeNo, double amt, String body, String ip,
    ...
    @Override
    public Map<String, String> getNativePayInfo(String productId, String outTradeNo, double amt, String body, String ip,
                                                String callbackUrl) throws WxErrorException {
        Map<String, String> packageParams = new HashMap<>();
        packageParams.put("appid", this.wxMpService.getWxMpConfigStorage().getAppId());
        packageParams.put("mch_id", this.wxMpService.getWxMpConfigStorage().getPartnerId());
    ...
    @Override
    public Map<String, String> getJsapiPaySign(String prepayId) {

        if (prepayId == null || prepayId.equals("")) {
            throw new RuntimeException("Failed prepay id is null.");
        }
    ...
    public Map<String, String> getPayInfo(Map<String, String> parameters) throws WxErrorException {
        ...
        if (!"SUCCESS".equalsIgnoreCase(wxMpPrepayIdResult.getReturn_code())
            ...
            error.setErrorMsg("return_code:" + wxMpPrepayIdResult.getReturn_code() + ";return_msg:"
                    + wxMpPrepayIdResult.getReturn_msg() + ";result_code:" + wxMpPrepayIdResult.getResult_code()
                    + ";err_code" + wxMpPrepayIdResult.getErr_code() + ";err_code_des"
    ...
    public WxMpPayResult getJSSDKPayResult(String transactionId, String outTradeNo) {
        ...
        packageParams.put("sign",
                WxCryptUtil.createSign(packageParams, this.wxMpService.getWxMpConfigStorage().getPartnerKey()));
    ...
    @Override
    public WxMpPayCloseResult closeOrder(String outTradeNo) {
        String nonce_str = System.currentTimeMillis() + "";
        ...
        HttpPost httpPost = new HttpPost("https://api.mch.weixin.qq.com/pay/closeorder");
        if (this.httpProxy != null) {
            RequestConfig config = RequestConfig.custom().setProxy(this.httpProxy).build();
            httpPost.setConfig(config);
        }
        ...
        try (CloseableHttpResponse response = this.wxMpService.getHttpclient().execute(httpPost)) {
            String responseContent = Utf8ResponseHandler.INSTANCE.handleResponse(response);
            XStream xstream = XStreamInitializer.getInstance();
            xstream.alias("xml", WxMpPayCloseResult.class);
            return (WxMpPayCloseResult) xstream.fromXML(responseContent);
        } catch (IOException e) {
            throw new RuntimeException("Failed to query order due to IO exception.", e);
        }
    }
    ...
    public WxMpPayRefundResult refundPay(Map<String, String> parameters) throws WxErrorException {
        ...
        try (CloseableHttpResponse response = this.wxMpService.getHttpclient().execute(httpPost)) {
            ...
            e.printStackTrace();
            String message = MessageFormatter.format("Exception happened when sending refund '{}'.", request.toString())
                    .getMessage();
     ...
     */
    public WxMpPayQueryRefundState queryRefundPay(Map<String, String> parameters) throws WxErrorException {
        SortedMap<String, String> refundParams = new TreeMap<>(parameters);
        refundParams.put("appid", this.wxMpService.getWxMpConfigStorage().getAppId());
        refundParams.put("mch_id", this.wxMpService.getWxMpConfigStorage().getPartnerId());
        refundParams.put("nonce_str", System.currentTimeMillis() + "");
        ...
        try (CloseableHttpResponse response = this.wxMpService.getHttpclient().execute(httpPost)) {
            String responseContent = Utf8ResponseHandler.INSTANCE.handleResponse(response);
            System.out.println("退款结果查询微信返回：" + responseContent);
            XStream xstream = XStreamInitializer.getInstance();
            xstream.processAnnotations(WxMpPayQueryRefundState.class);
            WxMpPayQueryRefundState wxMpPayRefundResult = (WxMpPayQueryRefundState) xstream.fromXML(responseContent);
    ...
    public WxRedpackResult sendRedpack(Map<String, String> parameters) throws WxErrorException {
        ...
        try (CloseableHttpResponse response = this.wxMpService.getHttpclient().execute(httpPost)) {
            ...
            String message = MessageFormatter.format("Exception occured when sending redpack '{}'.", request.toString())
                    .getMessage();
            this.log.error(message, e);
            throw new WxErrorException(WxError.newBuilder().setErrorMsg(message).build());
        } finally {
