...
import com.shangji.user.entity.SessionUser;
...
public class SjSessionUserService {
     ...
     */
    public void initSessionUser(User user, HttpServletRequest request){
        if (user == null) {
     ...
     *  客户端登录初始化 
     */
    public void customerInitSessionUser(User user, HttpServletRequest request,String accessToken){
        if (user == null) {
            throw new SystemException("登录用户为空，初始化失败");
        }
        SessionUser su = new SessionUser();
...
//        }
    ...
    public boolean checkCookieUser(HttpServletRequest request){
      //如果session为空，则取Cookie验证
        ...
        User user = userService.checkCookieLogin(loginId, pwd);
        if (user == null) {
