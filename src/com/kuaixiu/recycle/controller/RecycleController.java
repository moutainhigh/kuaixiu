...
//            items = sb.toString();
    ...
    public void getProductId(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String savePath = serverPath(request.getServletContext().getRealPath("")) + System.getProperty("file.separator") +SystemConstant.IMAGE_PATH;
            log.info("图片保存路径:" + savePath);
            String modelUrl = getProjectUrl(request) + "/../images/" + imageUrl.substring(imageUrl.lastIndexOf("/") + 1);
            //先判断文件是否存在  不存在则下载
