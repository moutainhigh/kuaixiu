...
import com.kuaixiu.order.dao.FromSystemMapper;
 ...
 * @version: V 1.0
 */
@Service("fromSystemService")
public class FromSystemService extends BaseService<FromSystem> {
    private static final Logger log= Logger.getLogger(FromSystemService.class);

    @Autowired
    private FromSystemMapper<FromSystem> mapper;


    public FromSystemMapper<FromSystem> getDao() {
        return mapper;
    }
     ...
     */
    public List<FromSystem> queryByName(String name){
    ...
    public boolean checkProjectName(String name){
        List<FromSystem> list = getDao().queryByName(name);
     ...
     */
    @Transactional
    public int update(FromSystem f){
    	if(f == null || f.getId() == null){
            throw new SystemException("参数为空，无法更新");
        }
        FromSystem project = getDao().queryById(f.getId());
        if(project == null){
     ...
     */
    @Transactional
    public int delete(FromSystem f){
        if(f == null || f.getId() == null){
            throw new SystemException("参数为空，无法更新");
        }
        FromSystem t = getDao().queryById(f.getId());
        t.setIsDel(1);
    ...
    public int deleteById(String idStr, SessionUser su){
        ...
        for(String id : ids){
        	FromSystem t = getDao().queryById(id);
            t.setIsDel(1);
