...
//存值方法,新的值添加在首位
function setHistoryItems(keyword,keyid,imghref,brandid,brandname) {
    var _localStorage = localStorage,
        historyItems = _localStorage.historyItems;

    if (historyItems === undefined) {
        localStorage.historyItems = [keyword,keyid,imghref,brandid,brandname];
    } else {
        historyItems = [keyword,keyid,imghref,brandid,brandname] + '|' + historyItems.split('|').filter(function (e) {
            return e != keyword+','+keyid+','+imghref+','+brandid+','+brandname;
        }).join('|');
