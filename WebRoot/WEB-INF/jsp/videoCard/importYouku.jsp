...
<link rel="stylesheet" href="${webResourceUrl}/resource/order/css/order.detail.css">
      <div class="am-cf am-padding am-padding-bottom-0">
        <div class="am-fl am-cf" style="width: 100%;">
          <strong class="am-text-primary am-text-lg">优酷卡密管理</strong> / <small>批量导入</small>
        </div>
      </div>
      ...
      <div class="am-g">
	    <form id="importForm" class="form-horizontal" role="form">
	      ...
	      <div class="form-group">
            ...
            <div class="col-sm-9">
              <p class="form-control-static">1、点击 <a href="${ctx}/file/download.do?fileId=40" target="_blank">这里</a> 打开Excel表格，在Excel表格中录入数据。</p>
            </div>
          </div>
          <div class="form-group">
...
function uploadModelExcel(){
    ...
    $.ajaxFileUpload({
        url: "${ctx}/videoCard/startImportYouku.do",
        type: "post",
        ...
        success: function(result){
        	...
        	if(result.success && result.data && result.data.pass){
        		...
        		$("#import_report_id").html("");
        		$("#error_panel").hide();
        	}else{
        		AlertText.tips("d_alert", "提示", "导入失败，请查看下方错误信息！");
...
function errorMsg(data){
	var rhtml = "";
 	if(!data.continueNext){
 		rhtml+="<span class=\"h4 text-danger\">" + data.error + "！</span>";
    }else if(!data.pass){
